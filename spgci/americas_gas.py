...
class AmericasGas:
    ...
    def get_tariff_rate_data(
        ...
        *,
        ...
        date_retire: Optional[datetime] = None,
        date_retire_lt: Optional[datetime] = None,
        date_retire_lte: Optional[datetime] = None,
        date_retire_gt: Optional[datetime] = None,
        date_retire_gte: Optional[datetime] = None,
        ...
        ----------
         ...
         date_effective_lte: Optional[date], optional
             filter by `date_effective <= x`, by default None
         date_retire: Optional[datetime], optional
             The date when the rate is retired or no longer applicable., by default None
         date_retire_gt: Optional[datetime], optional
             filter by `date_retire > x`, by default None
         date_retire_gte: Optional[datetime], optional
             filter by `date_retire >= x`, by default None
         date_retire_lt: Optional[datetime], optional
             filter by `date_retire < x`, by default None
         date_retire_lte: Optional[datetime], optional
             filter by `date_retire <= x`, by default None
        ...
        filter_params.append(list_to_filter("dateRetire", date_retire))
        if date_retire_gt is not None:
            filter_params.append(f'dateRetire > "{date_retire_gt}"')
        if date_retire_gte is not None:
            filter_params.append(f'dateRetire >= "{date_retire_gte}"')
        if date_retire_lt is not None:
            filter_params.append(f'dateRetire < "{date_retire_lt}"')
        if date_retire_lte is not None:
            filter_params.append(f'dateRetire <= "{date_retire_lte}"')
